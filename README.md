...
黑名单每行一条，按正文包含文本匹配，忽略英文大小写和空行，不支持正则表达式。单聊普通生成和重新生成完成并收尾后，命中任一条就重新生成，替换命中回复，不增加候选回复或重复发送用户消息。默认每轮合计最多额外重试 3 次，可设为 1 到 10 次；黑名单重试间隔 1.5 秒，等待期间可用酒馆停止按钮取消。总次数耗尽仍命中时，丢弃该回复并保存删除结果。

流式和非流式均支持；不扫描历史聊天或独立思维链字段，不处理续写、候选回复、群聊和后台工具生成。开启 ST 原生自动切换回复时，本功能暂停以避免重复触发。手动停止、切换聊天、修改消息或黑名单设置会取消当前重试链。

后台生成模式下会先确认旧任务已清理，清理或保存失败时停止重试并提示。检测依赖当前页面运行，关闭页面后不会在服务端继续执行黑名单检测；流式内容也可能先显示再被替换，不能撤销其他插件已处理的内容。
